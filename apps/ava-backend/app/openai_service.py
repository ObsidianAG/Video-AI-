...
class OpenAIServiceError(RuntimeError):
    """Raised when storyboard generation fails closed."""
...
class OpenAIStoryboardService:
    ...
    def _create_storyboard_sync(self, request: StoryboardRequest) -> StoryboardPlan:
        try:
            ...
            raise OpenAIServiceError("OpenAI storyboard generation failed.") from exc
        parsed = response.output_parsed
        if parsed is None:
            raise OpenAIServiceError("OpenAI returned no structured storyboard.")
        return parsed
