...
function isBelowThreshold(bucket) {
    if (bucket.limit === 0)
        return false;
 ...
 *   x-request-id
 */
export class RateLimitParser extends EventEmitter {
    _latestState = null;
    get latestState() {
     ...
     */
    parse(headers, httpStatus) {
        const h = (name) => headers.get(name) ?? null;
        const intOrZero = (v) => v !== null ? (parseInt(v, 10) || 0) : 0;
        const nullableDate = (v) => {
            if (!v)
                return null;
        ...
        const retryAfterSeconds = retryAfterRaw !== null ? (parseFloat(retryAfterRaw) || 0) : null;
        const raw = {
            requests: {
    ...
    lowestRemainingPct() {
        ...
        if (!s)
            return null;
        const buckets = [s.requests, s.tokens, s.inputTokens, s.outputTokens];
        const pcts = buckets
            .filter((b) => b.limit > 0)
